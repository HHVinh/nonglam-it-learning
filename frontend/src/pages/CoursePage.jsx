  ...
  const getCourseMeta = () => {
    switch(courseType) {
      case "A": return { 
        ...
        color: "blue", docLink: "https://drive.google.com/file/d/1Lr1i028NQ9SQYEluo61I4AAijGC_DvQl/view", quizLink: "/quiz/entry?subject=A"
      };
      case "B": return { 
        title: "TIN B (Phần 2)", sub: "Excel nâng cao", 
        color: "green", docLink: "https://drive.google.com/file/d/1ATK7dDKnj-9GDd-wwdT8PXCixA6SlGr8/view", quizLink: "/quiz/entry?subject=B"
      };
      case "ACCESS": return { 
      ...
      <div className="bg-white dark:bg-slate-800 border-b border-slate-100 dark:border-slate-800 sticky top-16 z-40">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-2.5 sm:py-3 flex items-center justify-between gap-2 sm:gap-3">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <Link to="/" className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-xl text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-800 transition-all duration-150 shrink-0">
              <ArrowLeft className="w-4 h-4 sm:w-3.5 sm:h-3.5" /> <span className="hidden sm:inline">Trở về</span>
            </Link>
            <div className="w-px h-5 bg-slate-200 dark:bg-slate-700 shrink-0" />
            <div className="min-w-0 flex flex-col justify-center">
              <h1 className="font-bold text-slate-900 dark:text-white text-xs sm:text-sm leading-tight uppercase truncate" style={{ fontFamily: "'Poppins', sans-serif" }}>
                {meta.title}
              </h1>
              <span className={cn("text-[10px] sm:text-xs font-medium truncate mt-0.5", colors.badge.split(" ").slice(-2).join(" "))}>
                {meta.sub}
              </span>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            ...
            {meta.quizLink && (
              <Link to={meta.quizLink}
                className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl text-[11px] sm:text-xs font-bold bg-amber-100 text-amber-800 hover:bg-amber-200 transition-colors border border-amber-200 shadow-sm shrink-0">
                <Star className="w-3.5 h-3.5 fill-amber-500 shrink-0" /> <span className="hidden sm:inline">Ôn trắc nghiệm</span><span className="sm:hidden">Trắc nghiệm</span>
              </Link>
            )}
          </div>
        </div>
